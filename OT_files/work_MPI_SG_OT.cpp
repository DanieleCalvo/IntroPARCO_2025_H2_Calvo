...
void matTransposeMPI(float* local_matrix, float* local_transpose, int rows_per_process, int N, int rank, int size) {
    ...
    bool isSymmetric = checkSymMPI(local_matrix, rows_per_process, N, rank, size);
    if (rank == 0) {
    ...
    for (int i = 0; i < rows_per_process; i++) {
        for (int j = 0; j < N; ++j) {
            local_transpose[j * rows_per_process + i] = local_matrix[i * N + j];
        }
    }
}

void printMatrix(float* matrix, int rows, int cols, FILE* file, const char* label) {
    fprintf(file, "%s:\n", label);
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            fprintf(file, "%.4f ", matrix[i * cols + j]);
        }
        fprintf(file, "\n");
    }
    fprintf(file, "\n");
...
void printValues(double* results, FILE* file, int n_times, int N, int size, float* M, float* T) {
    ...
    fprintf(file, "\nMatrices:\n");
    printMatrix(M, N, N, file, "Original Matrix M in Scatter:");
    printMatrix(T, N, N, file, "Transposed Matrix T in Scatter:");
}
