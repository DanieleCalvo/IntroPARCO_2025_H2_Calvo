...
void matTransposeMPI(float** M, float** T, int N, int rank, int size) {
    int rows_per_process = (N + size - 1) / size; // Number of rows per process
    int start_row = rank * rows_per_process;
    int end_row = std::min(start_row + rows_per_process, N);

    // Transpose assigned rows into a local buffer
    float** local_T = allocate_matrix(N);
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            local_T[i][j] = 0.0f; // Initialize
        }
    }
    ...
    for (int i = 0; i < N; ++i) {
        MPI_Reduce(local_T[i], T[i], N, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
    }

    free_matrix(local_T, N);
